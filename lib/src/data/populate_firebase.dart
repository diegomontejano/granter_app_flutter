...
import 'package:granter_app/src/data/firebase.dart';
...
class PopulateFirebase {
  ...
  Future uploadFiles() async {
    // original _products.json
    final List _productsJson = [
      {
        "title": "Brown eggs",
        "type": "dairy",
    ...
    for (dynamic _object in _productsJson) {
      // get _filename from _object
      final String _fileName = _object["filename"];

      // get _imageURL from _fileName in Firebase Storage
      final String _imageURL = await firebase_storage.FirebaseStorage.instance
          .ref("images/$_fileName")
          .getDownloadURL();

      // get _document.id from Firebase Firestore
      final _document = _firebase.collectionProducts.doc();

      // upload _object to Firebase Firestore with attributes below
      _document.set({
        "title": _object["title"],
        "type": _object["type"],
        ...
        "width": _object["width"],
        "price": _object["price"],
        "rating": _object["rating"],
        "created":
            DateTime.now().toString(), // add attribute "created:timestamptz"
        "id": _document.id, // add attribute "id:document.id"
        "imageURL": _imageURL, // add attribute "imageURL:imageURL"
      });
    }
  }
}
